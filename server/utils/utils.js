module.exports = db => {
  ...
  function logError(user, action, err, res, req) {
    ...
    err.user = { id_user: user.id, id_unit: user.id_unit, current_month: user.current_month, id_employee: user.id_employee, identification: user.identification };
    if (req?.params) err.params = req.params;
    if (req?.body) err.body = req.body;
    let detail = err ? JSON.stringify(err, null, 4) : '';
    detail = detail.replace(/\\\\/g, '\\').replace(/\\"/g, '"').replace(/\\n\s/g, '\n        ');
    db.models.LogError.create({ id_user: user ? user.id : null, action: action, error: err ? err.toString() : '', detail }).then(() => {
      if (res) {
        res.status(400);
        res.end();
      }
